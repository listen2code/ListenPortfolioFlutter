...
> [!NOTE]
> **Implementation Status**: 本文档描述的 JWT 验证、审批工作流以及 `resume_requests` 数据模型目前属于**架构设计草案 (Proposed Design)**。由于个人简历展示场景的需求变化，当前客户端代码库中暂未实装该模块（`ResumeIntent` 和 `ResumeState` 保持了最简实现）。本文档保留以供未来升级安全授权或企业级部署时参考。
